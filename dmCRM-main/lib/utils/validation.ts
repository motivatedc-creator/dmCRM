...
export const inventorySchema = z.object({
  ...
  // Capture the cost price paid to acquire this item. This is
  // separate from the selling price (`unit_price`) and is used
  // for profitability analysis. Existing records may omit
  // cost_price; treat missing values as zero or mirror unit_price.
...
export const invoiceItemSchema = z.object({
  ...
  unit_price: z.coerce.number().min(0),
  // Optional cost price used to snapshot the cost at time of invoicing.
  cost_price: z.coerce.number().min(0).optional(),
  item_type: z.enum(['service', 'part', 'labor']),
});

// Schema used to validate new vendor submissions. Vendors are
// suppliers or service providers you purchase from, distinct from
// your customers. Only the name is required; other contact fields
// are optional to accommodate varying levels of detail.
